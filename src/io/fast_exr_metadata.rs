...
impl FastEXRParser {
    pub fn from_file(path: &Path) -> anyhow::Result<Self> {
        ...
        if file_size > 1024 * 1024 {
            // >1MB files use memory mapping for header
    ...
    pub fn parse_metadata(
        &mut self,
    ) -> Result<FastEXRMetadata, Box<dyn std::error::Error + Send + Sync>> {
        // Check magic number
            ...
            match attr_name.as_str() {
                ...
                _ => {
                    ...
                    if attr_size > 0 && attr_size <= 64 {
                        // Only small, likely text attributes
    ...
    fn read_fixed_string(&mut self, size: usize) -> anyhow::Result<String> {
        ...
        let result =
            String::from_utf8_lossy(&self.data[self.position..self.position + size]).to_string();
        self.position += size;
        Ok(result)
    }
...
pub fn read_exr_metadata_ultra_fast(path: &Path) -> anyhow::Result<FastEXRMetadata> {
    ...
    parser
        .parse_metadata()
        .map_err(|e| anyhow::anyhow!("Failed to parse EXR metadata: {}", e))
}
...
impl From<FastEXRMetadata> for crate::io::exr_metadata::ExrMetadata {
    fn from(fast_meta: FastEXRMetadata) -> Self {
        use crate::io::exr_metadata::{ExrMetadata, LayerMetadata, MetadataGroup};
        ...
        let mut groups = vec![MetadataGroup {
            name: "Ogólne".into(),
            items: general_items,
        }];
        ...
        if !fast_meta.custom_attributes.is_empty() {
            let custom_items: Vec<(String, String)> =
                fast_meta.custom_attributes.into_iter().collect();
        ...
        let layer_name = fast_meta.layer_name.unwrap_or_default();
