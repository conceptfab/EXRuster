...
fn read_and_group_metadata_fast(path: &Path) -> anyhow::Result<ExrMetadata> {
    ...
    general_items.push((
        "Rozmiar pliku".into(),
        human_size(fs::metadata(path).map(|m| m.len()).unwrap_or(0)),
    ));
    ...
    header_items.push((
        "display_window".into(),
        format!("{:?}", fast_meta.display_window),
    ));
    header_items.push((
        "pixel_aspect".into(),
        format!("{:.3}", fast_meta.pixel_aspect),
    ));
    ...
    groups.push(MetadataGroup {
        name: "Grupy kanałów".into(),
        items: channel_group_items,
    });
...
fn read_and_group_metadata_standard(path: &Path) -> anyhow::Result<ExrMetadata> {
    ...
    let shared = &meta
        .headers
        .first()
        .ok_or_else(|| anyhow::anyhow!("Brak nagłówków w pliku EXR"))?
        .shared_attributes;
    header_items.push((
        "display_window".into(),
        format!("{:?}", shared.display_window),
    ));
    ...
    for (raw_name, value) in shared.other.iter() {
        ...
        if let Some(existing) = header_items
            .iter_mut()
            .find(|(k, _)| k.eq_ignore_ascii_case(&normalized_key))
        {
    ...
    for header in meta.headers.iter() {
        ...
        let mut layer_items: Vec<(String, String)> =
            Vec::with_capacity(header.own_attributes.other.len());
        for (raw_name, value) in header.own_attributes.other.iter() {
            let name_lower = raw_name.to_string().to_ascii_lowercase();
            let normalized_key =
                if name_lower == "pixel_aspect_ratio" || name_lower == "pixel_aspect" {
                    "pixel_aspect".to_string()
                } else if name_lower == "chromaticities" {
                    "chromaticities".to_string()
                } else if name_lower == "time_code" || name_lower == "timecode" {
        ...
        layers.push(LayerMetadata {
            ...
            width: w,
            height: h,
            attributes: layer_items,
        });
    }

    // Posortuj warstwy: najpierw bez nazwy (bazowa), potem alfabetycznie
    layers.sort_by(|a, b| match (a.name.is_empty(), b.name.is_empty()) {
...
pub fn build_ui_lines(meta: &ExrMetadata) -> Vec<String> {
    ...
    for layer in &meta.layers {
        let title = if layer.name.is_empty() {
            "(domyślna)".to_string()
        } else {
            layer.name.clone()
        };
        out.push(format!(
            "Warstwa: {}  — {}x{}",
            title, layer.width, layer.height
        ));
...
pub fn build_ui_rows(meta: &ExrMetadata) -> Vec<(String, String)> {
    ...
    for layer in &meta.layers {
        ...
        rows.push((
            "Wymiary".into(),
            format!("{}x{}", layer.width, layer.height),
        ));
        for (k, v) in &layer.attributes {
            let key = k.trim();
            let pretty_k = if key.is_empty() {
                "Atrybut".to_string()
            } else {
                key.to_string()
            };
