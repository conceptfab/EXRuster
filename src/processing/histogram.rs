...
mod tests {
    ...
    fn test_histogram_performance() {
        ...
        let pixels: Vec<f32> = (0..pixel_count * 4)
        ...
        println!(
            "Histogram computation for {}MP took {:?}",
    ...
    fn test_luminance_standards() {
        ...
        assert!(
            (rec601_lum - rec709_lum).abs() > 1e-6,
            "Rec.601 and Rec.709 should give different results: {} vs {}",
            rec601_lum,
            rec709_lum
        );
        ...
        assert!(
            (sum601 - 1.0).abs() < 1e-6,
            "Rec.601 coefficients should sum to 1.0, got {}",
            sum601
        );
        assert!(
            (sum709 - 1.0).abs() < 1e-6,
            "Rec.709 coefficients should sum to 1.0, got {}",
            sum709
        );
    }
