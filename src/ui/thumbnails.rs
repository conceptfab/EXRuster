...
use slint::{ComponentHandle, Image, ModelRc, Rgba8Pixel, SharedPixelBuffer, VecModel, Weak};
use std::path::Path;
...
pub fn load_thumbnails_for_directory(
    ...
    if let Some(ui) = ui_handle.upgrade() {
        push_console(
            &ui,
            &console,
            format!("[folder] loading thumbnails: {}", directory.display()),
        );
        ...
        std::thread::spawn(move || {
            ...
            prog.set(
                0.1,
                Some(&format!(
                    "📁 Found {} EXR files, starting processing...",
                    total_files
                )),
            );
            ...
            let thumbnail_works = match crate::io::thumbnails::generate_thumbnails_cpu_raw(
                ...
                Err(e) => {
                    ...
                    slint::invoke_from_event_loop(move || {
                        if let Some(ui) = ui_weak_clone.upgrade() {
                            ui.set_status_text(
                                format!("❌ Failed to generate thumbnails: {}", e).into(),
                            );
            ...
            slint::invoke_from_event_loop(move || {
                if let Some(ui) = ui_weak_clone.upgrade() {
                    ...
                    prog.finish(Some(&format!(
                        "✅ Successfully loaded {} thumbnails in {} ms",
                        count_clone, ms_clone
                    )));

                    // Log to console in separate call - using simple status text
                    ui.set_status_text(
                        format!(
                            "[folder] {} EXR files | thumbnails in {} ms",
                            count_clone, ms_clone
                        )
                        .into(),
                    );
